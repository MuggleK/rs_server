...
class Rshu4:
    ...
    def get_ts(self):
        res = self.session.get(self.ts_url, proxies=self.proxy, timeout=30)
...
def run(base_url, ts_url, cookie_s, cookie_t, proxy=None):
    ...
    for _ in range(3):
        try:
            rs4 = Rshu4(base_url, ts_url, cookie_s, cookie_t, proxy)
            rs4_text, cookies = rs4.verify()
            if rs4_text:
                ...
                return {
                    'msg': 'success',
                    'code': 200,
                    'data': {'html': rs4_text, 'cookie': cookies}
                }
        except:
...
if __name__ == '__main__':
    # cookie_s = 'FSSBBIl1UgzbN7N443S'
    # cookie_t = 'FSSBBIl1UgzbN7N443T'
    # base_url = 'https://kjt.hubei.gov.cn/kjdt/tzgg/index.shtml'
    # ts_url = 'https://uis.cqut.edu.cn/4QbVtADbnLVIc/c.FxJzG50F.844807d.js'
    # print(run(base_url, ts_url, cookie_s, cookie_t, proxy=None))
    pass
