...
function get_param_from_index(indexCode, jsFileCode) {
    ...
    let arr380Regexp = /(_\$.{2})(.push\((?:_\$[\w\$]+,? ?){300,}\);)/gm;
    let arr380RepStr = 'window.arr380=$1;window.arr380$2;window.bigFunc=window.arr380[205]();return;';
...
function calCookie(html){
    ...
    let {cookie, suffix} = getCookie(cd, vars_arr, arr380_42, bigFunc);
...
function calSuffix(html, checkPath, postData) {
    ...
    let indexCode = parse(html).querySelector("script").innerText;
    let {cd, vars_arr, arr380_42, bigFunc} = get_param_from_index(indexCode, jsFileCode);
    let {cookie, suffix} = getCookie(cd, vars_arr, arr380_42, bigFunc, postData, checkPath);
